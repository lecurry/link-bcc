...
def sedreadwoval():
    while 1:
        ...
        try:
            global sedwoval
            sedNowlist = list(set(sedwoval))
            DataHandle().tcpSend(sedNowlist)
            sedwoval = []
...
def tcp_send():
    try:
        _thread.start_new_thread( sedwaitwoval, () )
        _thread.start_new_thread( sedreadwoval, () )
    except:
        print ("Error: 无法启动线程")